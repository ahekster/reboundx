...
void rebx_register_default_params(struct rebx_extras* rebx){
    ...
    rebx_register_param(rebx, "Rcentral", REBX_TYPE_DOUBLE);
    rebx_register_param(rebx, "mu_effcentral", REBX_TYPE_DOUBLE);
}
...
struct rebx_force* rebx_load_force(struct rebx_extras* const rebx, const char* name){
    ...
    else if (strcmp(name, "quadrupole") == 0){
        force->update_accelerations = rebx_quad_force;
        force->force_type = REBX_FORCE_POS;
    }
    else{
        char str[300];
